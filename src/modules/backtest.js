...
module.exports = class Backtest {
  ...
  async getBacktestPairs() {
    ...
    const currencyPeriodPairs = await new Promise(resolve => {
      const pairs = [];
      this.instances.symbols.forEach(symbol => {
        this.exchangeCandleCombine.fetchCandlePeriods(symbol.exchange, symbol.symbol).then(periods => {
          pairs.push({
            name: `${symbol.exchange}.${symbol.symbol}`,
            options: periods.length > 0 ? periods : ['15m', '1m', '5m', '1h', '4h']
          });

          if (pairs.length === this.instances.symbols.length) {
            resolve(pairs);
          }
        });
      });
    ...
    return currencyPeriodPairs.sort((a, b) => {
      const x = a.name;
      const y = b.name;
    ...
    return new Promise(async (resolve, reject) => {
      ...
      const mockedRepository = {
        fetchCombinedCandles: async (mainExchange, symbol, period, exchanges = []) => {
          ...
          Object.keys(periodCache[key]).forEach(ex => {
      ...
      const candles = exchangeCandles[exchange]
        ...
        .map(candle => {
          ...
          Object.entries(dates).forEach(([time]) => {
            if (time >= candle.time) {
              signalMap = dates[time].map(i => {
                return {
  ...
  getBacktestSummary(signals, initial_capital) {
    return new Promise(async resolve => {
      ...
      const trades = {
        ...
        total: 0, // Total number of Trades
